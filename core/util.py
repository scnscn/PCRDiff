...
def get_rgb(image): # 训练时输入为tiff, CHW,输出png
...
def get_rgb2(image): # 训练时的输入为png，包括使用image read pretiff的, CHW
    image = image.mul(0.5).add_(0.5)
    image = image.mul(255.0).clamp_(0, 255.0)
    image = image.permute(1, 2, 0).cpu().detach().numpy() # HWC
    image = image.astype(np.uint16)
...
def tensor2tiff(image):   #训练输入为tiff且保存tiff结果
    image = image.mul(0.5).add_(0.5)  #chw
    image = image.mul(10000).clamp_(0, 10000)
    image = image.permute(1, 2, 0).cpu().detach().numpy() # HWC
    image = image.astype(np.uint16)
    r = image[:, :, 0]
    g = image[:, :, 1]
...
def tensor2img(tensor, out_type=np.uint8, min_max=(-1, 1)): # 可视化v2.0，切换tiff与png输入时需要更改get_rgb
    '''
...
def postprocess_tiff(images):
    return [tensor2tiff(image) for image in images]
