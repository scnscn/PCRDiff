...
class Sen2_MTC_Old_Multi(data.Dataset): #old数据集
    def __init__(self, data_root, mode='train'):
        ...
        if mode == 'train':
            self.tile_list = np.loadtxt(
                os.path.join(self.data_root, 'train.txt'), dtype=str)
        elif mode == 'val':
        ...
        for tile in self.tile_list:
            image_name_list = [image_name.split('.')[0] for image_name in os.listdir(
                os.path.join(self.data_root, 'Sen2_MTC_Old', tile, 'cloudless'))]
                
            
            for image_name in image_name_list:
                image_cloud_path0 = os.path.join(
                    self.data_root, 'Sen2_MTC_Old', tile, 'cloud', image_name + '_0.jpg')
                image_cloud_path1 = os.path.join(
                    self.data_root, 'Sen2_MTC_Old', tile, 'cloud', image_name + '_1.jpg')
                image_cloud_path2 = os.path.join(
                    self.data_root, 'Sen2_MTC_Old', tile, 'cloud', image_name + '_2.jpg')
                image_cloudless_path = os.path.join(
                    self.data_root, 'Sen2_MTC_Old', tile, 'cloudless', image_name + '.jpg')
    ...
    def __getitem__(self, index):
        ...
        #image_read_rgb when input is png
        image_cloud0 = self.image_read_rgb(cloud_image_path0)
        image_cloud1 = self.image_read_rgb(cloud_image_path1)
        image_cloud2 = self.image_read_rgb(cloud_image_path2)
        image_cloudless = self.image_read_rgb(cloudless_image_path)
    ...
    def image_read_rgb(self, image_path):
        # 读取PNG图像
...
class Sen2_MTC_New_Multi(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        image_cloud0 = self.image_read_pretiff(cloud_image_path0)
        image_cloud1 = self.image_read_pretiff(cloud_image_path1)
        image_cloud2 = self.image_read_pretiff(cloud_image_path2)
        image_cloudless = self.image_read_pretiff(cloudless_image_path)
        '''
    ...
    def image_read_pretiff(self, image_path):  #提前从tif分布转为png分布
    ...
    def image_read(self, image_path):
        ...
        if std.ndim == 1:
            std = std.view(-1, 1, 1)
        image.sub_(mean).div_(std) #[0-10000]->[0,1]->[-1,1]
