...
class Palette(BaseModel):
    ...
    def set_input(self, data):
        ...
        self.input_image = self.set_device(data.get('input_image')) if data.get('input_image') is not None else None
    ...
    def train_step(self):
        ...
        self.flag=0
        for id,train_data in enumerate(tqdm.tqdm(self.phase_loader)):
            self.iter += self.batch_size
            self.writer.set_iter(self.epoch, self.iter, phase='train')
            self.set_input(train_data)
          
            self.optG.zero_grad()
            loss = self.netG(self.gt_image, self.cond_image, mask=self.mask,flag=self.flag)
            if self.flag==0:
                self.flag=1
            else:
                self.flag=0
