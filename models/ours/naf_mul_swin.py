...
from models.ours.swin_models import SwinTransformerBlock
from einops import rearrange
import math
from models.ours.nafnet_double_encoder_splitcaCond_splitcaUnet import EmbedBlock, EmbedSequential
from pdb import set_trace as stx
...
class CondNAFBlock(nn.Module):
    ...
    def forward(self, inp):
        ...
        x = self.sg(x)
        x_avg2, x_max2 = x.chunk(2, dim=1)
        x_avg2 = self.sca_avg(x_avg2)*x_avg2
        x_max2 = self.sca_max(x_max2)*x_max2
        x = torch.cat([x_avg2, x_max2], dim=1)
...
class NAFSWINBlock(EmbedBlock):
    def __init__(self, c, DW_Expand=2, FFN_Expand=2, drop_out_rate=0., input_size=16):
        super().__init__()
        dw_channel = c * DW_Expand
        self.conv1 = nn.Conv2d(in_channels=c, out_channels=dw_channel,
                               kernel_size=1, padding=0, stride=1, groups=1, bias=True)
        ...
        self.conv3 = nn.Conv2d(in_channels=dw_channel // 2, out_channels=c,
                               kernel_size=1, padding=0, stride=1, groups=1, bias=True)
        self.swin = EmbedSequential(
                    *[SwinTransformerBlock(dim = c, input_resolution = (input_size, input_size), num_heads = 4, 
                    window_size=8, shift_size=0 if i%2==0 else 4 ,
                    mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                    act_layer=nn.GELU, norm_layer=nn.LayerNorm) for i in range(4)]
                )
    ...
    def forward(self, inp, t):
        ...
        temp_x = x
        r=x.shape[2]
        temp_x = rearrange(temp_x,'b c h w -> b (h w) c')
        temp_x = self.swin(temp_x,(r,r))
        temp_x =rearrange(temp_x,'b (h w) c -> b c h w',h=r)
        temp_x = self.norm11(temp_x)
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.time_emb_middle = nn.ModuleList()
        self.time_emb_up = nn.ModuleList()
        ...
        for num in enc_blk_nums: 
            self.encoders.append(
                EmbedSequential(
                    *[NAFSWINBlock(chan) for _ in range(num)]
                )
            )
        ...
        self.middle_blks = EmbedSequential(
                *[NAFSWINBlock(chan) for _ in range(middle_blk_num)]
            )  #一个
        self.time_emb_middle = nn.Sequential(
            nn.SiLU(),
            nn.Linear(256, chan)
        )
    ...
    def forward(self, inp, gammas):
        t = self.map(self.emb(gammas.view(-1, ))) #t:b,256
        ...
        for encoder, down, cond_encoder, cond_down, t_emb in zip(self.encoders, self.downs, self.cond_encoders, self.cond_downs, self.time_emb_down):
        ...
        for decoder, up, enc_skip, t_emb in zip(self.decoders, self.ups, encs[::-1], self.time_emb_up):
