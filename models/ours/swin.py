...
class NAFSWINBlock(EmbedBlock):
    def __init__(self, c, DW_Expand=2, FFN_Expand=2, drop_out_rate=0., input_size=16):
        ...
        self.swin = EmbedSequential(
                    *[SwinTransformerBlock(dim = dw_channel, input_resolution = (input_size, input_size), num_heads = 4, 
        ...
        self.norm1 = LayerNorm2d(c)
        self.norm2 = LayerNorm2d(c)
        self.norm11=LayerNorm2d(dw_channel)
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.middle_blks = EmbedSequential(
                *[NAFSWINBlock(chan, input_size=input_size) for _ in range(middle_blk_num)]
            )  #一个
    ...
    def forward(self, inp, gammas):
        ...
        for encoder, down, cond_encoder, cond_down, t_emb in zip(self.encoders, self.downs, self.cond_encoders, self.cond_downs, self.time_emb_down):
            '''
            x=rearrange(x,'b c h w -> b (h w) c')
            x = encoder(x, (int(x.shape[1]**0.5), int(x.shape[1]**0.5)),t_emb(t)) 
        ...
        for decoder, up, enc_skip, t_emb in zip(self.decoders, self.ups, encs[::-1], self.time_emb_up):
            ...
            x = x + enc_skip
            x=rearrange(x,'b c h w -> b (h w) c')
            x = decoder(x, (int(x.shape[1]**0.5), int(x.shape[1]**0.5)), t_emb(t))
            x=rearrange(x,'b (h w) c -> b c h w', h=int(x.shape[1]**0.5))
            '''
            x = up(x)
            x = x + enc_skip
